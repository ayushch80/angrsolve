...
from angrsolve.strategies import get_strategies
...
def explore(
    ...
    # Try registered strategies before symbolic exploration.
    for strategy in get_strategies():
        if strategy.detect(proj):
            logger.info("[+] Using strategy: %s", strategy.name)
            result = strategy.solve(proj, input_setup, cfg)
